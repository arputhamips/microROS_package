...
#include <nav_msgs/msg/odometry.h> // [ADDED] Odometry message type
...
// 3. ROBOT CONSTANTS
// ==========================================
// REV HD Hex Motor base is 28 counts/rev. 
#define TICKS_PER_REV 1120.0 
#define WHEEL_DIA 0.045      // [UPDATED] Sprocket Diameter (Meters)
#define WHEEL_BASE 0.30      // [TODO] Measure distance between tread centers (Meters)
#define MAX_PWM 255
...
// Motor Control Logic (Differential Drive)
void set_motor_speeds(float linear_x, float angular_z) {
  // Simple kinematic mixing for tank drive
  ...
  // Drive Left
  if (left_pwm >= 0) {
    digitalWrite(PIN_M_LEFT_DIR, HIGH); // Forward
    analogWrite(PIN_M_LEFT_PWM, left_pwm);
  } else {
    digitalWrite(PIN_M_LEFT_DIR, LOW); // Reverse
    analogWrite(PIN_M_LEFT_PWM, -left_pwm);
  }

  // Drive Right
  if (right_pwm >= 0) {
    digitalWrite(PIN_M_RIGHT_DIR, LOW); // Forward
    analogWrite(PIN_M_RIGHT_PWM, right_pwm);
  } else {
    digitalWrite(PIN_M_RIGHT_DIR, HIGH); // Reverse
    analogWrite(PIN_M_RIGHT_PWM, -right_pwm);
  }
}

// Subscription Callback (Input from Pi)
void cmd_vel_callback(const void * msgin) {
  const geometry_msgs__msg__Twist * msg = (const geometry_msgs__msg__Twist *)msgin;
  set_motor_speeds(msg->linear.x, msg->angular.z);
}

// Timer Callback (Output to Pi)
void timer_callback(rcl_timer_t * timer, int64_t last_call_time) {
  ...
  // Convert to Radians (Joint States)
  ...
  // Sync Time (Used for all messages)
  ...
  // --- [ADDED] 2. CALCULATE ODOMETRY ---
  // Distance traveled by each tread in meters
  ...
  // Frame IDs are set in setup() to avoid crashes
  ...
  if (bno08x.getSensorEvent(&sensorValue)) {
    ...
    switch (sensorValue.sensorId) {
      ...
      case SH2_ACCELEROMETER:
        imu_msg.linear_acceleration.x = sensorValue.un.accelerometer.y;
        imu_msg.linear_acceleration.y = -sensorValue.un.accelerometer.x;
        imu_msg.linear_acceleration.z = sensorValue.un.accelerometer.z;
        break;
        
      case SH2_GYROSCOPE_CALIBRATED:
        imu_msg.angular_velocity.x = sensorValue.un.gyroscope.y;
        imu_msg.angular_velocity.y = -sensorValue.un.gyroscope.x;
...
void setup() {
  ...
  // [ADDED] 2. Odometry Publisher
  rclc_publisher_init_default(
    &odom_pub, &node, ROSIDL_GET_MSG_TYPE_SUPPORT(nav_msgs, msg, Odometry),
    "odom/unfiltered"
  );

  // [ADDED] Odometry Static Init (Prevent Crash)
